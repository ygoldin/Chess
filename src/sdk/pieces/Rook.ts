...
import { rookEvalBlack, rookEvalWhite } from '../boardEvaluation';

export class Rook implements ChessPiece {
    public static readonly VALUE = 50;
    public static readonly SYMBOL = 'R';
    ...
    public getValue = (row: number, column: number) => {
        if (this.color === PieceColor.White) {
            return Rook.VALUE + rookEvalWhite[row][column];
        } else {
            return -1 * (Rook.VALUE + rookEvalBlack[row][column]);
        }
    };

    public getSymbol = () => Rook.SYMBOL;
